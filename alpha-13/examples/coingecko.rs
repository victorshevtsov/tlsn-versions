...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let uri: &str = "/api/v3/simple/price?ids=bitcoin&vs_currencies=usd";
    let api_key: &str = &env::var("API_KEY").expect("API_KEY is required");

    let extra_headers = vec![("x-cg-demo-api-key", api_key)];
...
async fn prover<S: AsyncWrite + AsyncRead + Send + Sync + Unpin + 'static>(
    ...
    let server_host: &str = "api.coingecko.com";
