...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let uri: String =
        env::var("URI").unwrap_or("/api/v3/simple/price?ids=bitcoin&vs_currencies=usd".into());
    println!("URI: {}", uri);
...
async fn notarize(uri: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let notary_port: u16 = env::var("NOTARY_PORT")
        ...
        .unwrap_or(7047);
    let server_host = "api.coingecko.com";
    let server_port = 443;

    let api_key: String = env::var("API_KEY").expect("API_KEY is required");
