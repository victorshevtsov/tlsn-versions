...
pub const MAX_RECV_DATA: usize = 2 << 20;
...
async fn main() -> Result<()> {
    ...
    let uri: String = env::var("URI").unwrap_or("https://example.com".into());
    ...
    prover(prover_socket, request_tx, attestation_rx, &uri).await?;

    tracing::info!("Elapsed: {:?}", start.elapsed());

    println!("{}\t elapsed {:.2}s", uri, start.elapsed().as_secs_f64());
...
async fn prover<S: AsyncWrite + AsyncRead + Send + Sync + Unpin + 'static>(
    ...
    let server_host = &uri[Position::BeforeHost..Position::AfterHost];
    ...
    let (tls_connection, prover_fut) = prover
        .connect(
            TlsClientConfig::builder()
                .server_name(ServerName::Dns(server_host.try_into()?))
    ...
    let (attestation, secrets) =
        notarize(prover, &request_config, req_tx, resp_rx, server_host).await?;

    // Write the attestation to disk.
    let attestation_path = "example-coingecko.attestation.tlsn";
    let secrets_path = "example-coingecko.secrets.tlsn";
